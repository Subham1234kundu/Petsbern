...
export default function PetDetailsPage() {
  ...
  useEffect(() => {
    async function fetchPet() {
      ...
      if (petData) {
        setPet(petData);
        setActiveImage(petData.main_image || "https://images.unsplash.com/photo-1552053831-71594a27632d?q=80&w=600&h=600&auto=format&fit=crop");
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans">
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              {pet?.breed_groups && (() => {
                ...
                if (petGroups.length === 0) return null;
                
                return (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {petGroups.map((group: string) => (
                      <span key={group} className="inline-flex items-center px-3 py-1 bg-yellow-50 border border-[#FFC501]/40 rounded-full text-[11px] font-bold text-yellow-800 tracking-wide uppercase">
                        {group.replace(' Group', '')}
                      </span>
                    ))}
                  </div>
      ...
      <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 pb-16">
        <PersonalityHighlights
          ...
          titleClassName="text-center text-[36px] font-normal text-black mb-6"
        />
