...
import { BIRD_CHARACTERISTICS, matchesCharacteristics } from '@/lib/breedCharacteristics';
...
export default function ExoticBirdsCategoryPage() {
  ...
  const displayBreeds = groupedPets.filter(pet => {
    ...
    if (selectedSize === "X-Small" && !(w <= 0.25)) return false;
    if (selectedSize === "Small"   && !(w > 0.25 && w <= 0.5)) return false;
    if (selectedSize === "Medium"  && !(w > 0.5 && w <= 1.0)) return false;
    if (selectedSize === "Large"   && !(w > 1.0 && w <= 2.0)) return false;
    if (selectedSize === "X-Large" && !(w > 2.0)) return false;

    if (!matchesCharacteristics(pet, selectedCharacteristics, BIRD_CHARACTERISTICS)) return false;
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 pt-6 pb-4">
          <div className="flex items-center justify-between gap-6">
            <div className="inline-flex p-1 bg-[#F3F4F6] rounded-full w-full sm:w-auto overflow-x-auto whitespace-nowrap">
              ...
              <Link href="/pet-categories/cats" className="flex-1 sm:flex-initial text-center px-4 md:px-8 py-2 rounded-full bg-transparent text-[#4F4F4F] text-[13px] sm:text-sm font-bold hover:text-black transition-all active:scale-95">Cats</Link>
              <Link href="/pet-categories/exotic-birds" className="flex-1 sm:flex-initial text-center px-4 md:px-8 py-2 rounded-full bg-black text-white text-[13px] sm:text-sm font-bold shadow-sm transition-all active:scale-95">Exotic Birds</Link>
            </div>
            <button
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-8">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <CharacteristicsFilter
              characteristics={BIRD_CHARACTERISTICS}
              ...
              displayBreeds.map((bird) => {
                ...
                return (
                  <BreedCardFull
                    key={bird.id}
                    name={bird.breed}
                    image={bird.main_image || '/images/labrador.png'}
                    weight={bird.weight}
