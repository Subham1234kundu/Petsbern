...
export default function ExoticBirdDetailsPage() {
  ...
  useEffect(() => {
    async function fetchPet() {
      ...
      if (petData) {
        setPet(petData);
        setActiveImage(petData.main_image || "https://images.unsplash.com/photo-1452570053594-1b985d6ea890?q=80&w=600&h=600&auto=format&fit=crop");
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-screen bg-white font-sans items-center justify-center">
        ...
        <p className="mt-4 text-gray-500 font-medium">Loading bird details...</p>
      </div>
    );
  }
  ...
  if (!pet && !loading) {
    return (
      <div className="flex flex-col min-h-screen bg-white font-sans items-center justify-center">
        <h1 className="text-3xl font-bold mb-4">Bird Not Found</h1>
        <p className="text-gray-500 mb-8">We couldn't find the exotic bird you're looking for.</p>
        <Link href="/pet-categories/exotic-birds" className="bg-[#FFC501] text-black px-8 py-3 rounded-full font-bold">
          Go Back to Exotic Birds
        </Link>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans">
      ...
      <div className="w-full bg-[#F2F4F5] h-[72px] flex items-center">
        <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 flex items-center gap-2">
          ...
          <Link href="/pet-categories/exotic-birds" className="text-[#5F6C72] text-[14px] hover:text-[#8B5E3C]">Our Pets</Link>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M9 18L15 12L9 6" stroke="#77878F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>

          <Link href="/pet-categories/exotic-birds" className="text-[#5F6C72] text-[14px] hover:text-[#8B5E3C]">Exotic Birds</Link>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M9 18L15 12L9 6" stroke="#77878F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
      ...
      {/* Main Content */}
      <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 flex flex-col lg:flex-row gap-12 pt-8">

        {/* Left Section: Images */}
        <div className="relative w-full lg:w-[600px] h-[400px] sm:h-[500px] lg:h-[600px]">
          {/* Thumbnails Overlay */}
          {thumbnails.length > 1 && (
            <div className="absolute top-4 left-4 z-10 flex flex-col gap-3">
              {thumbnails.map((thumb, idx) => (
                <div
                  key={idx}
                  className={`w-[60px] h-[60px] sm:w-[80px] sm:h-[80px] overflow-hidden border-2 cursor-pointer transition-all shadow-md ${activeImage.includes(thumb.split('&')[0]) ? 'border-[#8B5E3C]' : 'border-white opacity-80 hover:opacity-100'
                    }`}
          ...
          {/* Main Image */}
          <div className="w-full h-full overflow-hidden border border-[#E4E7E9] rounded-2xl">
        ...
        <div className="flex-1 flex flex-col gap-8">
          ...
          {/* Info Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-8 gap-x-4 border-b border-[#F2F4F5] pb-10">
            {/* Column 1 items */}
            <div className="flex flex-col gap-8">
              <div>
                <p className="text-[#686363] text-[13px] sm:text-[14px] mb-0.5 font-normal">Gender</p>
                <p className="text-[#1E1E1E] text-[16px] sm:text-[18px] font-normal">{pet?.gender || "Female & Male"}</p>
              </div>
              <div>
                <p className="text-[#686363] text-[13px] sm:text-[14px] mb-0.5 font-normal">Vaccinated</p>
                <p className="text-[#1E1E1E] text-[16px] sm:text-[18px] font-normal">{pet?.vaccinated || "Yes"}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-8">
              {!isBreedPage && (
                <div>
                  ...
                  <p className="text-[#1E1E1E] text-[16px] sm:text-[18px] font-normal">{pet?.age || "Young"}</p>
                </div>
              )}
              <div>
                <p className="text-[#686363] text-[13px] sm:text-[14px] mb-0.5 font-normal">Location</p>
                <p className="text-[#1E1E1E] text-[16px] sm:text-[18px] font-normal">{pet?.location || "Available for Delivery"}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-8 col-span-2 sm:col-span-1">
              <div>
                ...
                <p className="text-[#1E1E1E] text-[16px] sm:text-[18px] font-normal">{pet?.color || "Multi-color"}</p>
              </div>
              {pet?.weight > 0 && (
                <div>
                  <p className="text-[#686363] text-[13px] sm:text-[14px] mb-0.5 font-normal">Weight</p>
                  <p className="text-[#1E1E1E] text-[16px] sm:text-[18px] font-normal">{pet.weight} kg</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-wrap items-center gap-4">
              <a
                ...
                className="w-full sm:w-[193px] bg-black text-white h-[48px] rounded-full text-[15px] font-medium hover:bg-gray-900 transition-all flex items-center justify-center shadow-md active:scale-95"
              >
                Call Us
              </a>
              ...
              <a
                ...
                className="w-full sm:w-[225px] bg-[#E4E7E9] text-black h-[48px] rounded-full text-[15px] font-medium hover:bg-gray-300 transition-all flex items-center justify-center shadow-sm active:scale-95"
              >
                WhatsApp Now
              </a>
            </div>
            ...
            <div className="flex items-center gap-1 mt-2">
              <span className="text-[#FFC107] font-bold text-[14px]">80+</span>
              <span className="text-[#5F6C72] text-[14px]">People Viewing This Bird</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 pt-16 pb-16">
        <div className="w-full border border-[#E4E7E9] rounded-2xl flex flex-wrap md:flex-nowrap items-center justify-between bg-white py-8 px-4 sm:px-6 shadow-sm">
          {/* Delivery */}
      ...
      {relatedPets.length > 0 && (
        <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 pb-16">
          <h2 className="text-black text-[24px] sm:text-[30px] font-semibold mb-10">
            {isBreedPage ? `Available ${petName} Birds` : `More ${pet?.breed || 'Birds'} Like ${petName}`}
          </h2>
        ...
        <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 md:mb-24 gap-6">
            <div className="max-w-[700px]">
              <h2 className="text-black font-normal leading-tight mb-4 text-[26px] md:text-[36px]" style={{ fontFamily: "var(--font-public-sans), sans-serif" }}>
                Experience the <span className="text-[#FFC501]">Pets Barn</span> Promise of Pawsome Care
              </h2>
              <p className="text-[#6E6E6E] leading-relaxed text-[15px] md:text-[18px]" style={{ fontFamily: "var(--font-public-sans), sans-serif" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              },
              {
                ...
                description: "Every pet undergoes a thorough health check by certified veterinarians to ensure they are active and healthy.",
                image: "/images/exAbout2.png"
              },
              {
                title: "Verified Lineage",
                description: "Providing documented lineage and vaccination certification to guarantee the authenticity and safety of your pet.",
      ...
      <section className="bg-white w-full">
        <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-12 mb-14">
          <h2 className="text-black text-[28px] sm:text-[36px] font-normal text-center mb-10">
            Bringing Your Pet Home in <span className="text-[#FFC501]">3 Simple Steps</span>
          </h2>

          <div className="flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-2 relative">
            {/* Step 1 */}
            <div className="w-full max-w-[356px] h-[356px] border border-[#C8C8C8] rounded-xl p-8 flex flex-col items-center text-center justify-center bg-white shadow-sm">
      ...
      <section className="py-20 md:py-24 bg-[#F6F5F2] w-full">
        <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12">
          {/* Header Area */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
            <div className="max-w-[700px]">
              <h2 className="text-black font-normal leading-tight mb-4 text-[28px] md:text-[36px]">
                Why Choose <span className="text-[#FFC501]">Pet Barns</span>?
              </h2>
              <p className="text-[#4F4F4F] leading-relaxed text-[15px] md:text-[16px]">
            ...
            <Link href="/contact-us" className="bg-black text-white px-8 py-3 rounded-full text-[14px] font-semibold hover:bg-gray-800 transition-all shadow-md active:scale-95">
              Contact Us
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              >
                ...
                <h3 className="text-black font-medium leading-snug text-[20px] md:text-[22px]">
                  {box.title}
                </h3>
              </div>
            ))}
      ...
      <section className="relative w-full h-[350px] md:h-[400px] overflow-hidden flex items-center">
        <div
        ...
        <div className="relative z-10 w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12 flex flex-col md:flex-row justify-between items-center gap-8">
          <h2 className="text-white font-normal leading-tight max-w-[700px] text-[28px] md:text-[42px]" style={{ fontFamily: "var(--font-public-sans), sans-serif" }}>
            Let us help you discover the one that fits you perfectly.
          </h2>
          <Link href="/contact-us" className="bg-white text-black rounded-full h-[50px] w-[267px] flex items-center justify-center font-semibold transition-all hover:bg-gray-100 shadow-xl text-[16px] md:text-[18px]" style={{ fontFamily: "var(--font-public-sans), sans-serif" }}>
            Contact Us
          </Link>
        </div>
      </section>
